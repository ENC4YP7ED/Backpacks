...
public class EntityInteractionEvents{
    ...
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        // If no backpack in chest slot, check accessories slot (if mod is loaded)
        if (!item.is(BPItems.BACKPACK) && ModList.get().isLoaded("accessories")) {
            item = getBackpackFromAccessories(target);
        }
